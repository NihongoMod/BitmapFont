...
public class BitmapFontRenderer {
	...
	//private FloatBuffer matrix = ByteBuffer.allocateDirect(16 * 4).asFloatBuffer();BufferUtils.createFloatBuffer
	...
	protected int renderString(String str, int x, int y, int color, boolean darken) {
		...
		if (str != null) {
			...
			GL11.glColor4f((float) (color >> 16 & 0xff) / 255F,
					(float) (color >> 8 & 0xff) / 255F, (float) (color & 0xff) / 255F,
		...
		for (int i = 0; i < str.length(); i++) {
			...
			if (c == '\247' && i + 1 < str.length()) {
				...
				if (j < 16) {
					...
					GL11.glColor3f((float)(col >> 16) / 255f, (float)(col >> 8 & 0xff) / 255f, (float)(col & 0xff) / 255f);
				} else {
